...
public class EquilibriumIndex {
    public static void main(String[] args) {
        ...
        A.add(2);
        A.add(3);
        System.out.println(solve(new ArrayList<Integer>(Arrays.asList(B))));
    ...
    public static int solve(ArrayList<Integer> A) {
        ...
        for(int i = 1; i<A.size(); i++){
            psList.add(i,A.get(i) + psList.get(i-1));
        }
        int size = psList.size()-1;
        for(int j = 0; j<psList.size(); j++){
            if(j == 0){
                sumL = 0;
                //sumR = psList.get(size); -----wrong as this will take sum from index 0 not from index 1
                sumR = psList.get(size) - psList.get(j);
            } else if (j==size) {
                sumR = 0;
                sumL = psList.get(j-1);
            } else{
    ...
    public static int optimizedSolve1(ArrayList<Integer> A) {
        ArrayList<Integer> psList = new ArrayList<>();
        psList.add(A.get(0));
        int sumR = 0;
        int sumL = 0;
        ...
        for(int j = 0; j<psList.size(); j++){
            ...
            sumR = psList.get(lastIndex) - psList.get(j);
            if(sumL == sumR){
                return j;
            }
        }
